...
def plotting():
    ...
    ax2 = ax.twinx()
    ax.cla()
    ax.plot(xval, yval, marker="o", label="Losses")
    ax2.cla()
    ax2.plot(xval, yval2, marker="o", label="Rewards", color="red")
    ax.set_xlabel('Frames')
    ax.set_ylabel('Losses')
    ax2.set_ylabel('All Rewards')
    plt.legend(loc=0)
    plt.tight_layout()
...
for frame_idx in range(1, num_frames + 1):
    ...
    if len(replay_buffer) > replay_initial:
        loss = compute_td_loss(model, target_model,
                               batch_size, gamma, replay_buffer)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    ...
    if frame_idx % 10000 == 0 and len(replay_buffer) > replay_initial:
        ...
        xval.append(frame_idx)
        yval.append(np.mean(losses, 0)[1])
        yval2.append(np.mean(all_rewards[-10:], 0)[1])
        plotting()
